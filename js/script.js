...
function nextWaveSource() {
    ...
    if (selectedWaveSource == waveSources.length + 1) {
        ...
        waveSources.push(new WaveSource(round(Math.random() * (1 - 0.01) + 0.01, 2).toFixed(2), round(Math.random() * (1 - 0.01) + 0.01, 2).toFixed(2), 1.5, 1, 2));
...
var waveSources = [
    new WaveSource(0.2, 0.2, 1.5, 1, 2),
    new WaveSource(0.8, 0.2, 1.5, 1, 2)
];
...
var paramFunction = (u, v, target) => {
    ...
    for (i = 0; i < waveSources.length; i++) {
        ...
        if (time * waveVelocity >= dist) { ampl += waveSources[i].amplMax * Math.sin(2 * Math.PI * (time / period - dist / waveLength) + waveSources[i].phase * Math.PI); }
    }
